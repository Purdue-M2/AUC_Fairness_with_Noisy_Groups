...
def train(model, optimizer_theta, optimizer_lambda, optimizer_p_list, scheduler, X_train_n, train_writter, X_test, test_writter, seed, num_epochs, start_epoch):
    ...
    violation_list = []
    X_train, y_train, group_neg_loaders, group_pos_loaders, phats, _ = np2torch(X_train_n, train_writter, args.train_batchsize,args.device, get_loader=True)
    X_test, y_test,  _, _, phats_test, X_test_list = np2torch(X_test, test_writter, args.train_batchsize,args.device, get_loader=False)
    for epoch in range(start_epoch, num_epochs):
        ...
        for idx, (group_neg_loader, group_pos_loader) in enumerate(zip(zip(*group_neg_loaders), zip(*group_pos_loaders))):
            ...
            #compute constraint
            if args.method == 'auc':
                constraints = get_auc_constraints_dro(model, train_writter, yy_group, intersec_label, overall_loss, yy_batch)# 4,
                
            elif args.method == 'fpr':
                constraints = get_equal_tpr_and_fpr_constraints_dro(model, yy_batch, yy_pred)
                
            elif args.method == 'naive':
                constraints = get_auc_constraints(model, train_writter, yy_group, intersec_label, overall_loss, yy_batch)
        ...
        if (epoch+1) % 1 == 0:
            ...
            yy_pred_test = model(X_test).view(-1)
            group_num = X_test_list[0].shape[0] + X_test_list[1].shape[0]
            auc, eer, accuracy, ap, fpr, tpr = calculate_metrics_for_train(y_test, yy_pred_test)
            auc_groups = []
            for (i, j) in product(range(test_writter.n_groups), range(test_writter.n_groups)):
                ...
                y_group_i = y_test[indices_group_i]
                y_group_j = y_test[indices_group_j]
                yy_pred_group_i = yy_pred_test[indices_group_i]
                yy_pred_group_j = yy_pred_test[indices_group_j]
...
def main():
    ...
    for seed in args.seeds:
        ...
        optimizer_theta = SAM(theta_params, base_optimizer, lr=0.001, momentum=0.9, weight_decay=0.01)
